...
import { pgPoolSsl, withSslMode } from "./pg-ssl";

/** Which database backend is active. */
export type DbSource = "postgres" | "pglite";

/**
 * Resolve the server Postgres connection string.
 * Accepts common Vercel + Supabase integration names without inventing secrets.
 * Prefer pooled URLs (port 6543 / pooler host) on serverless.
 */
...
/**
 * Active backend: real **Postgres** (Supabase pooler / any Postgres URL) when a
 * connection string is set; otherwise embedded **PGLite** for local/preview only.
 * Production on Vercel always requires a Postgres URL — never PGLite.
 */
...
async function createSql(): Promise<Sql> {
  ...
  if (process.env.VERCEL) {
    throw new Error(
      "A Postgres connection string is required in production. " +
        "Set DATABASE_URL (preferred) or POSTGRES_URL to the Supabase pooler URI " +
        "(project qdeozgkmrqectbuhetvc). Do not use PGLite on Vercel.",
    );
  }
